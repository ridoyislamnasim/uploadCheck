...
const loginschema = require("../models/auth/auth/authModel");
const jwt = require("jsonwebtoken")
const studentschema = require("../models/auth/student/studentModel");
home = async (req, res) => {
...
login = async (req, res) => {
    ...
    if (!email && email.trim() === '') {
        ...
        try {
            ...
            const expiresIn = 86400000  // Default to one day if not provided

            jwt.sign(payload, "kjeFgH6%f/47823ehsd$%@/>tahv13212876543rvvf%$$", { expiresIn }, (err, token) => {
                if (err) throw err;
